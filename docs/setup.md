...
5. Set one complete Google or Microsoft client pair for sign-in. Configure the OpenAI-compatible provider in Settings → General after startup.
...
Configure a public HTTPS endpoint, provider API key, model ID, and token limits in Settings → General.
Save and test checks streaming and tool calling through the Agent bridge before saving.
CRM starts without provider configuration; AI sessions require a verified configuration.
Existing agent versions from gateway deployments require rebuilding after selecting the new provider.
The historical Cloudflare migration remains unchanged. The new migration only adds provider fields and preserves existing data.
